...
export async function loadAppDataFromSupabase(session: SaaSAuthSession, storeId: string): Promise<AppData | null> {
  const [
    ...
    fetchOptionalTable(session, "inventory_input_settings", storeId, (row): InventoryInputSetting => ({
      id: rowString(row, "id"),
      itemType: row.item_type as InventoryInputSetting["itemType"],
      itemId: rowString(row, "item_id"),
      inputMode: row.input_mode as InventoryInputSetting["inputMode"],
      createdAt: rowString(row, "created_at"),
      updatedAt: rowString(row, "updated_at"),
    })),
    fetchOptionalTable(session, "packaging_classifications", storeId, (row): PackagingClassification => ({
      id: rowString(row, "id"),
      ingredientId: rowString(row, "ingredient_id"),
      packagingRole: row.packaging_role as PackagingClassification["packagingRole"],
      brandImportance: row.brand_importance as PackagingClassification["brandImportance"],
      yearRoundUsage: row.year_round_usage as PackagingClassification["yearRoundUsage"],
      usageCategory: row.usage_category as PackagingClassification["usageCategory"],
      confidence: rowNumber(row, "confidence"),
      reason: rowString(row, "reason"),
      source: row.source as PackagingClassification["source"],
      createdAt: rowString(row, "created_at"),
      updatedAt: rowString(row, "updated_at"),
    })),
...
export async function saveAppDataToSupabase(session: SaaSAuthSession, storeId: string, data: AppData) {
  ...
  await Promise.all([
    ...
    replaceOptionalTableRows(session, "inventory_input_settings", storeId, data.inventoryInputSettings.map((item) => withStore({
      id: item.id,
      item_type: item.itemType,
      item_id: item.itemId,
    ...
    replaceOptionalTableRows(session, "packaging_classifications", storeId, data.packagingClassifications.map((item) => withStore({
      id: item.id,
      ingredient_id: item.ingredientId,
      packaging_role: item.packagingRole,
      brand_importance: item.brandImportance,
      year_round_usage: item.yearRoundUsage,
      usage_category: item.usageCategory,
